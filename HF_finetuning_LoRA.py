...
audio_folder = "./All_Segments/"  
transcript_folder = "./All_Segments/" 
...
print(dataset_dict["test"][0])  # Check the first data sample
...
model_name_or_path = f"openai/whisper-{model_name}"
task = "transcribe"
language = 'de'
...
dataset_dict = dataset_dict.map(prepare_dataset, remove_columns=dataset_dict.column_names["train"], num_proc=16)
...
# from whisper.normalizers import EnglishTextNormalizer  # OpenAI's text normalizer
# from helper import normalize_text as proposed_normalizer
# from whisper.normalizers import EnglishTextNormalizer 


eval_dataloader = DataLoader(dataset_dict["test"], batch_size=16, collate_fn=data_collator)
...
f = open(f"./HF_finetune_{model_name}_{learning_rate}_metrics.txt", "a")
f.write(f"Before Finetuning\n")
...
def compute_metrics(pred):
    ...
    references = [label.strip() for label in label_str]

    basic_normalized_predictions = [basic_normalizer(pred).strip() for pred in pred_str]
    basic_normalized_references = [basic_normalizer(label).strip() for label in label_str]
    number_normalized_predictions = [number_normalize(pred).strip() for pred in pred_str]
    number_normalized_references = [number_normalize(label).strip() for label in label_str]
    opAI_normalized_predictions = [whisper_normalize(pred).strip() for pred in pred_str]
    opAI_normalized_references = [whisper_normalize(label).strip() for label in label_str]
    proposed_1_normalized_predictions = [proposed_1(pred).strip() for pred in pred_str]
    proposed_1_normalized_references = [proposed_1(label).strip() for label in label_str]
    proposed_2_normalized_predictions = [proposed_2(pred).strip() for pred in pred_str]
    proposed_2_normalized_references = [proposed_2(label).strip() for label in label_str]
    proposed_3_normalized_predictions = [proposed_3(pred).strip() for pred in pred_str]
    proposed_3_normalized_references = [proposed_3(label).strip() for label in label_str]
    ...
    return {"wer": wer, "basic_normalized_wer": basic_normalized_wer, "number_normalized_wer": number_normalized_wer, "opAI_normalized_wer": opAI_normalized_wer, "proposed_1_normalized_wer": proposed_1_normalized_wer, "proposed_2_normalized_wer": proposed_2_normalized_wer, "proposed_3_normalized_wer": proposed_3_normalized_wer}
...
training_args = Seq2SeqTrainingArguments(
    output_dir=f"./checkpts/whisper-{model_name_or_path}-de-LoRA-split-{learning_rate}",  # change to a repo name of your choice
...
trainer = Seq2SeqTrainer(
    ...
    # compute_metrics=lambda pred: {"validation_loss": -pred.loss},
